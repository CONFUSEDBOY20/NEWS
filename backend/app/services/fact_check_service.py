...
class FactCheckService:
        ...
        try:
            async with httpx.AsyncClient(timeout=8.0, follow_redirects=True) as client:
                ...
                if resp.status_code == 200:
                    ...
                    # Strip tags
                    clean_p = re.findall(r"<p[^>]*>(.*?)</p>", html, re.IGNORECASE | re.DOTALL)
    ...
    async def _run_pipeline(
        ...
        # Step 2: Search internal FactDB
        db_evidence = await self.fact_db.search_fact_database(primary_claim)

        # Step 3: Search external trusted sources
        web_evidence = await self.search_provider.search_trusted_sources(primary_claim, limit=4)

        # Step 4: Combine & Deduplicate evidence items
        all_evidence: List[EvidenceItem] = []
        seen_urls = set()
        for ev in db_evidence + web_evidence:
        ...
        response = FactCheckResponse(
            ...
            verification_providers=["TruthLens FactDB", "Wire Source Graph", "Institutional Verification Matrix"],
