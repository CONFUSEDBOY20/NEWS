...
import { Globe, Search, ExternalLink, ShieldCheck, Clock, Loader2, ArrowUpRight, Sparkles } from "lucide-react";

export function WorldNewsView() {
  const { setInputPreload, setActiveView, setVerificationResult, t, language } = useApp();
  ...
  const loadNews = async () => {
    setLoading(true);
    try {
      const data = await api.getWorldNews(category);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-white/[0.06]">
        <div>
          <div className="flex items-center gap-2 text-indigo-400 text-xs font-mono font-semibold tracking-wider uppercase mb-1">
            ...
            <span>{t.newsHeaderWorld}</span>
          </div>
          <h2 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">
            International Wire & Verified Reports
          </h2>
        </div>
        ...
        <form onSubmit={handleSearch} className="relative w-full md:w-80">
          <input
            ...
            className="w-full bg-black/40 border border-white/[0.08] rounded-xl py-2 pl-9 pr-4 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500/60 focus:ring-1 focus:ring-indigo-500/20 font-normal transition-all"
          />
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-20 gap-3 text-slate-500">
          ...
          <span className="text-xs font-mono">Querying international news wire index...</span>
        </div>
      )}
            ...
            >
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between text-[11px] font-mono text-slate-400">
                  <span className="px-2 py-0.5 rounded-md bg-white/[0.04] text-indigo-300 border border-white/[0.06] font-semibold">
                    {art.source_name}
                  </span>
                ...
                {/* Title */}
                <h3 className="text-sm sm:text-base font-bold text-white group-hover:text-indigo-200 transition-colors leading-snug line-clamp-2">
                  {art.title}
                </h3>
              ...
              <div className="pt-4 border-t border-white/[0.06] flex items-center justify-between gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-300 text-xs font-semibold border border-indigo-500/20 transition-all group-hover:border-indigo-500/40"
                >
